class PagesController < ApplicationController
  ...
  def dashboard
    ...
    @garden_events = params[:start_date].nil? ? Event.where("farm_id = ? AND event_category = ? AND (date BETWEEN ? AND ?)", 1, "garden", Date.today.beginning_of_week, Date.today.end_of_week) : Event.where("farm_id = ? AND event_category = ? AND (date BETWEEN ? AND ?)", 1, "garden", Date.parse(params[:start_date]).beginning_of_week, Date.parse(params[:start_date]).end_of_week)
